...
- **PDF download stuck / popup blocked (three compounding issues)**:
  ...
  3. **`window.open()` after `await fetch()` is blocked by popup blocker**: Calling `window.open(url, '_blank')` after any `await` breaks the browser's user-gesture chain — popup blocker silently kills the new tab with no error. Fix: use a plain `<a href="/api/download/{id}" target="_blank">` (synchronous, no JS needed); API route returns `NextResponse.redirect(signedUrl)` (302) so the browser follows it automatically. **Never use `fetch → window.open` for file downloads.**
